/*
        name: jack tercheria
        date: 2/28/2025
        description: create example 5 29
*/
CREATE PROCEDURE sp_customer_search_dyn(in_customer_name VARCHAR(30),
...
BEGIN
        ...
        IF in_contact_firstname IS NOT NULL THEN 
                IF l_where_clause<>'WHERE' THEN
                        SET l_where_clause=CONCAT(l_where_clause,' AND '); 
                END IF;
                SET l_where_clause=CONCAT(l_where_clause, ' contact_firstname="',in_contact_firstname,'"');
                END IF;
        IF in_phoneno IS NOT NULL THEN 
                IF l_where_clause<>'WHERE' THEN
                SET l_where_clause=CONCAT(l_where_clause,' AND '); 
                END IF;
                SET l_where_clause=CONCAT(l_where_clause, ' phoneno="',in_phoneno,'"');
                END IF;


        SET @sql=CONCAT('SELECT * FROM customers ', l_where_clause, ";");
...
CALL sp_customer_search_dyn("McTavish", "CHAMBLISS", "ROMNEY", "(327) 244-36");